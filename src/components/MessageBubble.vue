  ...
  >
    <div class="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-10">
      ...
      <button 
        v-if="!isEditing"
        @click="startEditing"
        class="p-1 hover:bg-black/10 dark:hover:bg-white/10 rounded text-gray-500 dark:text-gray-400"
        title="Edit"
      >
        ✎
      </button>
    ...
    <template v-if="!isEditing && message.parts && message.parts.length > 0">
      <div v-for="(part, index) in message.parts" :key="part.id || index" class="mb-2">
        ...
        <div v-if="part.type === 'reasoning'" class="mb-3">
          <details class="group border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-800/50 overflow-hidden">
            <summary class="flex items-center gap-2 p-2 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700/50 select-none text-xs font-medium text-gray-500 uppercase tracking-wider">
              <span class="transform group-open:rotate-90 transition-transform">▶</span>
              Reasoning Process
            </summary>
            <div class="p-3 border-t border-gray-200 dark:border-gray-700 text-sm text-gray-600 dark:text-gray-300 bg-white dark:bg-gray-900/30 whitespace-pre-wrap font-mono">
              {{ part.content }}
            </div>
          </details>
        </div>
        ...
        <div v-else-if="part.type === 'tool-call' && part.toolCall" class="mb-3">
           <div class="border border-gray-300 dark:border-gray-600 rounded overflow-hidden bg-white/50 dark:bg-black/20">
            <details class="group">
              <summary class="flex items-center justify-between p-2 cursor-pointer hover:bg-black/5 dark:hover:bg-white/5 select-none">
                ...
                <div class="flex items-center gap-2">
                  <span v-if="getResult(part.toolCall.id)" class="text-xs px-1.5 py-0.5 rounded" :class="getResult(part.toolCall.id)?.isError ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' : 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400'">
              ...
              <div class="p-2 border-t border-gray-200 dark:border-gray-700 text-xs font-mono overflow-x-auto">
                <div class="mb-2">
                  ...
                  <pre class="bg-gray-50 dark:bg-gray-900 p-2 rounded">{{ JSON.stringify(part.toolCall.arguments, null, 2) }}</pre>
                </div>
                
                <div v-if="getResult(part.toolCall.id)">
                  <div class="font-semibold text-gray-500 mb-1">Result:</div>
                  <pre class="bg-gray-50 dark:bg-gray-900 p-2 rounded max-h-60 overflow-y-auto">{{ getResult(part.toolCall.id)?.result }}</pre>
                </div>
        ...
        <div v-else-if="part.type === 'text' && part.content">
           <div v-if="!showRaw" class="prose dark:prose-invert max-w-none" v-html="renderMarkdown(part.content)"></div>
           <pre v-else class="whitespace-pre-wrap font-mono text-sm text-gray-800 dark:text-gray-200">{{ part.content }}</pre>
        </div>
    ...
    <div v-if="message.toolCalls?.length" class="mb-3 space-y-2">
      <div v-for="call in message.toolCalls" :key="call.id" class="border border-gray-300 dark:border-gray-600 rounded overflow-hidden bg-white/50 dark:bg-black/20">
        <details class="group">
          <summary class="flex items-center justify-between p-2 cursor-pointer hover:bg-black/5 dark:hover:bg-white/5 select-none">
            <div class="flex items-center gap-2 font-mono text-sm">
            ...
            <div class="flex items-center gap-2">
              <span v-if="getResult(call.id)" class="text-xs px-1.5 py-0.5 rounded" :class="getResult(call.id)?.isError ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' : 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400'">
          ...
          <div class="p-2 border-t border-gray-200 dark:border-gray-700 text-xs font-mono overflow-x-auto">
            ...
            <div v-if="getResult(call.id)">
              <div class="font-semibold text-gray-500 mb-1">Result:</div>
              <pre class="bg-gray-50 dark:bg-gray-900 p-2 rounded max-h-60 overflow-y-auto">{{ getResult(call.id)?.result }}</pre>
            </div>
            <div v-else class="text-gray-500 italic">
              Waiting for result...
            </div>
    ...
    <!-- Loading Indicator -->
    <div v-if="message.role === 'assistant' && !message.content && !message.toolCalls?.length" class="flex items-center gap-1 h-6 px-2">
    ...
    <div v-if="message.content">
      <div v-if="isEditing" class="mt-2">
        <textarea 
          v-model="editContent" 
          class="w-full p-2 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-black/20 focus:outline-none focus:ring-2 focus:ring-blue-500 min-h-[100px]"
        ></textarea>
        <div class="flex justify-end gap-2 mt-2">
          <button @click="cancelEditing" class="px-3 py-1 text-sm rounded hover:bg-black/5 dark:hover:bg-white/5">Cancel</button>
          <button @click="saveEdit" class="px-3 py-1 text-sm bg-blue-600 text-white rounded hover:bg-blue-700">Save</button>
        </div>
      </div>
      <template v-else>
        <div v-if="!showRaw" class="prose dark:prose-invert max-w-none" v-html="renderedContent"></div>
        <pre v-else class="whitespace-pre-wrap font-mono text-sm text-gray-800 dark:text-gray-200">{{ message.content }}</pre>
      </template>
    </div>
    </template>
    ...
    <div v-if="message.attachments?.length" class="mt-3 flex flex-wrap gap-2">
      ...
      >
        <img 
          ...
          :src="att.content" 
          class="max-w-xs max-h-64 rounded object-contain" 
          :alt="att.name"
        />
